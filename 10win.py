...
def s0(p): return 2*p-p**2
def s1(p): return 1-p
def s2(p): return 1-p
def s3(p): return 1-p**2
def s4(p): return 2*p**2-2*p+1
def s5(p): return 1-p**2
def s6(p): return 1-p**2
def s7(p): return 2*p-2*p**2
def s8(p): return 1+p**2-p
def s9(p): return 1-p
...
csv_filename = "strategy_values_10win.csv"
...
# Customize the plot
plt.title("Probability Expressions for 10 Successful Outcomes", fontsize=16)
