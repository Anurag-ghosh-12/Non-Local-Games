...
# Define probability expressions for 16 strategies
def s0(p): return 2*p - 2*p**2
def s1(p): return p**2 + 1 - 2*p
def s2(p): return p**2 + 1 - 2*p
def s3(p): return 2*p - p**2
def s4(p): return p**2
def s5(p): return 1 - p**2
def s6(p): return 1 - p**2
def s7(p): return p**2
def s8(p): return p**2 - p + 1
def s9(p): return p - p**2
def s10(p): return p - p**2
def s11(p): return p**2 - p + 1
def s12(p): return p**2 + 1 - p
def s13(p): return p - p**2
def s14(p): return p - p**2
def s15(p): return p**2 - p + 1
...
# Format values up to 6 decimal places
df = df.applymap(lambda x: f"{x:.6f}")

# Save the DataFrame to a CSV file
df.to_csv('strategy_values_8winCHSH.csv')
...
# Customize the plot
plt.title("Probability Expressions for 8-win CHSH Successful Outcomes", fontsize=14)
plt.xlabel("p (Probability)", fontsize=12)
plt.ylabel("Strategy Output", fontsize=12)
